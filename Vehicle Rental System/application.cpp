...
void Application :: renderEditVehicleMenu() const{
    string header = "Enter registrtaion no.of vehicle you want to edit : ";
    ...
    try{
       ...
       if(newPrice != ""){
        ...
        this->db->updatedRecord(modifiedVehicle);

        this->showDialog("Vehicle data upadted successfully");
       }
    } catch(Error e){
        this->showDialog(e.getMessage());
    }
...
void Application :: renderAddNewTripMenu() const {
    ...
    for(auto &vehicle : freeVehicles){
        string registrationNo = vehicle->getRegistartionNumber();
...
void Application :: renderStartTripMenu() const{
    ...
    try{
        ...
        this->db->updatedRecord(modifiedTrip);
